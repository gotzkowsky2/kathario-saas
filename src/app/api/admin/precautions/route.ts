...
export async function POST(request: NextRequest) {
  try {
    ...
    const precaution = await prisma.precaution.create({
      data: { tenantId, title: String(title).trim(), content: String(content).trim(), workplace, timeSlot, priority, isActive: true },
    })
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const existing = await prisma.precaution.findFirst({ where: { id, tenantId: tenantId } })
