...
// 테넌트 격리 미들웨어 설정
// Prisma v5에서 $use 미들웨어가 제거되어 주석 처리
...
// 테넌트 격리를 위한 헬퍼 함수들
export function withTenantFilter<T extends Record<string, any>>(where: T = {} as T): T & { tenantId: string } {
  const tenantId = getCurrentTenantId()
  if (!tenantId) {
    throw new Error('테넌트 ID가 설정되지 않았습니다.')
  }
  return { ...where, tenantId }
}

export function withTenantData<T extends Record<string, any>>(data: T): T & { tenantId: string } {
  const tenantId = getCurrentTenantId()
  if (!tenantId) {
    throw new Error('테넌트 ID가 설정되지 않았습니다.')
  }
